...
class sfSimpleGoogleSitemap {
  ...
  public function processConfig()
  {
    ...
    if ($urls and is_array($urls))
    {
        ...
        foreach ($urls as $a)
        {
            $gsg->addUrl($controller->genUrl($a['url'], true), false, date('c'), false, $a['freq'], $a['priority']);
        }
    }
      ...
      foreach ($models as $just_a_name => $a)
      {
        ...
        switch ($orm)
        {
          ...
          default: // default use Propel
            $propel_method = (isset($a['propel_method'])) ? $a['propel_method'] : 'doSelect';
            $objects = $this->queryUsingPropel($model, $criteria_array, $order_by, $group_by, $limit, $propel_method);
        }
   ...
   * @return Object[]
   */
  public function queryUsingPropel($model_name, $criteria_array, $order_by, $group_by, $limit, $method)
  {
    ...
    if ($order_by)
    {
      $column = constant($order_by['peer'].'::'.$order_by['column']);
    ...
    // query records
    return call_user_func($peer.$method, $c);
  }
