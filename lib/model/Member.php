...
class Member extends BaseMember
{
   ...
   */
    ...
    public function getLastActivityWith($member_id)
    {
        ...
        $sql = '(SELECT ms.sender_id AS member_id, "mailed" AS activity, UNIX_TIMESTAMP(ms.created_at) AS dtime, IF(ms.sender_deleted_at, NULL, ms.thread_id) AS action_id, IF(ms.sender_deleted_at, UNIX_TIMESTAMP(ms.sender_deleted_at), null) AS deleted FROM message AS ms WHERE ms.type = 1 AND ms.sender_id = %MEMBER_ID% AND ms.recipient_id = %PROFILE_ID% )
                UNION
                (SELECT ms.sender_id, "mailed", UNIX_TIMESTAMP(ms.created_at), IF(ms.recipient_deleted_at, NULL, ms.thread_id), IF(ms.recipient_deleted_at, UNIX_TIMESTAMP(ms.recipient_deleted_at), null) FROM message AS ms WHERE ms.type = 1 AND ms.sender_id = %PROFILE_ID% AND ms.recipient_id = %MEMBER_ID% )
                UNION
                (SELECT w.member_id, "winked", UNIX_TIMESTAMP(w.created_at), NULL, NULL FROM wink AS w WHERE ((w.member_id = %MEMBER_ID% AND w.profile_id = %PROFILE_ID%) OR (w.member_id = %PROFILE_ID% AND w.profile_id = %MEMBER_ID% )) AND w.sent_box = 1 )
                UNION
                (SELECT h.member_id, "hotlisted", UNIX_TIMESTAMP(h.created_at), NULL, NULL FROM hotlist AS h WHERE (h.member_id = %MEMBER_ID% AND h.profile_id = %PROFILE_ID%) OR (h.member_id = %PROFILE_ID% AND h.profile_id = %MEMBER_ID%) )
                UNION
                (SELECT v.member_id, "visited", UNIX_TIMESTAMP(v.updated_at), NULL, NULL FROM profile_view AS v WHERE (v.member_id = %MEMBER_ID% AND v.profile_id = %PROFILE_ID%) OR (v.member_id = %PROFILE_ID% AND v.profile_id = %MEMBER_ID%) )
