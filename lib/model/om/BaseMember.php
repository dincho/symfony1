...
abstract class BaseMember extends BaseObject  implements Persistent {
	...
	protected $last_paypal_item;


	
	protected $paypal_unsubscribed_at;


	
	protected $last_paypal_payment_at;
	...
	public function getLastPaypalSubscrId()
	{

		return $this->last_paypal_subscr_id;
	}

	
	public function getLastPaypalItem()
	{

		return $this->last_paypal_item;
	}

	
	public function getPaypalUnsubscribedAt($format = 'Y-m-d H:i:s')
	{

		if ($this->paypal_unsubscribed_at === null || $this->paypal_unsubscribed_at === '') {
			return null;
		} elseif (!is_int($this->paypal_unsubscribed_at)) {
						$ts = strtotime($this->paypal_unsubscribed_at);
			if ($ts === -1 || $ts === false) { 				throw new PropelException("Unable to parse value of [paypal_unsubscribed_at] as date/time value: " . var_export($this->paypal_unsubscribed_at, true));
			}
		} else {
			$ts = $this->paypal_unsubscribed_at;
		}
		if ($format === null) {
			return $ts;
		} elseif (strpos($format, '%') !== false) {
	...
	public function getLastPaypalPaymentAt($format = 'Y-m-d H:i:s')
	{

		if ($this->last_paypal_payment_at === null || $this->last_paypal_payment_at === '') {
			return null;
		} elseif (!is_int($this->last_paypal_payment_at)) {
						$ts = strtotime($this->last_paypal_payment_at);
			if ($ts === -1 || $ts === false) { 				throw new PropelException("Unable to parse value of [last_paypal_payment_at] as date/time value: " . var_export($this->last_paypal_payment_at, true));
			}
		} else {
			$ts = $this->last_paypal_payment_at;
		}
		if ($format === null) {
			return $ts;
		} elseif (strpos($format, '%') !== false) {
	...
	public function setLastIp($v)
	{
		...
		if ($this->last_ip !== $v) {
			$this->last_ip = $v;
			$this->modifiedColumns[] = MemberPeer::LAST_IP;
		}
	...
	public function setLastPaypalSubscrId($v)
	{
		...
		if ($this->last_paypal_subscr_id !== $v) {
			$this->last_paypal_subscr_id = $v;
			$this->modifiedColumns[] = MemberPeer::LAST_PAYPAL_SUBSCR_ID;
		}

	} 
	
	public function setLastPaypalItem($v)
	{
		...
		if ($this->last_paypal_item !== $v) {
			$this->last_paypal_item = $v;
			$this->modifiedColumns[] = MemberPeer::LAST_PAYPAL_ITEM;
		}

	} 
	
	public function setPaypalUnsubscribedAt($v)
	{

		if ($v !== null && !is_int($v)) {
			$ts = strtotime($v);
			if ($ts === -1 || $ts === false) { 				throw new PropelException("Unable to parse date/time value for [paypal_unsubscribed_at] from input: " . var_export($v, true));
			}
		} else {
			$ts = $v;
		}
		if ($this->paypal_unsubscribed_at !== $ts) {
			$this->paypal_unsubscribed_at = $ts;
			$this->modifiedColumns[] = MemberPeer::PAYPAL_UNSUBSCRIBED_AT;
		}

	} 
	
	public function setLastPaypalPaymentAt($v)
	{

		if ($v !== null && !is_int($v)) {
			$ts = strtotime($v);
			if ($ts === -1 || $ts === false) { 				throw new PropelException("Unable to parse date/time value for [last_paypal_payment_at] from input: " . var_export($v, true));
			}
		} else {
			$ts = $v;
		}
		if ($this->last_paypal_payment_at !== $ts) {
			$this->last_paypal_payment_at = $ts;
			$this->modifiedColumns[] = MemberPeer::LAST_PAYPAL_PAYMENT_AT;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->last_ip = $rs->getInt($startcol + 37);

			$this->last_paypal_subscr_id = $rs->getString($startcol + 38);

			$this->last_paypal_item = $rs->getString($startcol + 39);

			$this->paypal_unsubscribed_at = $rs->getTimestamp($startcol + 40, null);

			$this->last_paypal_payment_at = $rs->getTimestamp($startcol + 41, null);

			$this->last_activity = $rs->getTimestamp($startcol + 42, null);

			$this->last_status_change = $rs->getTimestamp($startcol + 43, null);

			$this->last_flagged = $rs->getTimestamp($startcol + 44, null);

			$this->last_login = $rs->getTimestamp($startcol + 45, null);

			$this->last_winks_view = $rs->getTimestamp($startcol + 46, null);

			$this->last_hotlist_view = $rs->getTimestamp($startcol + 47, null);

			$this->last_profile_view = $rs->getTimestamp($startcol + 48, null);

			$this->last_activity_notification = $rs->getTimestamp($startcol + 49, null);
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[37] => $this->getLastIp(),
			$keys[38] => $this->getLastPaypalSubscrId(),
			$keys[39] => $this->getLastPaypalItem(),
			$keys[40] => $this->getPaypalUnsubscribedAt(),
			$keys[41] => $this->getLastPaypalPaymentAt(),
			$keys[42] => $this->getLastActivity(),
			$keys[43] => $this->getLastStatusChange(),
			$keys[44] => $this->getLastFlagged(),
			$keys[45] => $this->getLastLogin(),
			$keys[46] => $this->getLastWinksView(),
			$keys[47] => $this->getLastHotlistView(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[37], $arr)) $this->setLastIp($arr[$keys[37]]);
		if (array_key_exists($keys[38], $arr)) $this->setLastPaypalSubscrId($arr[$keys[38]]);
		if (array_key_exists($keys[39], $arr)) $this->setLastPaypalItem($arr[$keys[39]]);
		if (array_key_exists($keys[40], $arr)) $this->setPaypalUnsubscribedAt($arr[$keys[40]]);
		if (array_key_exists($keys[41], $arr)) $this->setLastPaypalPaymentAt($arr[$keys[41]]);
		if (array_key_exists($keys[42], $arr)) $this->setLastActivity($arr[$keys[42]]);
		if (array_key_exists($keys[43], $arr)) $this->setLastStatusChange($arr[$keys[43]]);
		if (array_key_exists($keys[44], $arr)) $this->setLastFlagged($arr[$keys[44]]);
		if (array_key_exists($keys[45], $arr)) $this->setLastLogin($arr[$keys[45]]);
		if (array_key_exists($keys[46], $arr)) $this->setLastWinksView($arr[$keys[46]]);
		if (array_key_exists($keys[47], $arr)) $this->setLastHotlistView($arr[$keys[47]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(MemberPeer::LAST_IP)) $criteria->add(MemberPeer::LAST_IP, $this->last_ip);
		...
		if ($this->isColumnModified(MemberPeer::LAST_PAYPAL_PAYMENT_AT)) $criteria->add(MemberPeer::LAST_PAYPAL_PAYMENT_AT, $this->last_paypal_payment_at);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setLastIp($this->last_ip);

		$copyObj->setLastPaypalSubscrId($this->last_paypal_subscr_id);

		$copyObj->setLastPaypalItem($this->last_paypal_item);

		$copyObj->setPaypalUnsubscribedAt($this->paypal_unsubscribed_at);

		$copyObj->setLastPaypalPaymentAt($this->last_paypal_payment_at);
