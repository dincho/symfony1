...
abstract class BaseBlock extends BaseObject  implements Persistent {
	...
	public function setMemberId($v)
	{
		...
		if ($this->member_id !== $v) {
			$this->member_id = $v;
			$this->modifiedColumns[] = BlockPeer::MEMBER_ID;
		}
	...
	public function setProfileId($v)
	{
		...
		if ($this->profile_id !== $v) {
			$this->profile_id = $v;
			$this->modifiedColumns[] = BlockPeer::PROFILE_ID;
		}
    ...
    foreach (sfMixer::getCallables('BaseBlock:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseBlock:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseBlock:save:pre') as $callable)
    {
    ...
    if ($this->isNew() && !$this->isColumnModified(BlockPeer::CREATED_AT))
    {
      $this->setCreatedAt(time());
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseBlock:save:post') as $callable)
    {
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(BlockPeer::MEMBER_ID)) $criteria->add(BlockPeer::MEMBER_ID, $this->member_id);
		if ($this->isColumnModified(BlockPeer::PROFILE_ID)) $criteria->add(BlockPeer::PROFILE_ID, $this->profile_id);
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseBlock:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseBlock::%s', $method));
    }
