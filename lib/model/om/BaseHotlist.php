...
abstract class BaseHotlist extends BaseObject  implements Persistent {
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = HotlistPeer::ID;
		}
	...
	public function setMemberId($v)
	{
		...
		if ($this->member_id !== $v) {
			$this->member_id = $v;
			$this->modifiedColumns[] = HotlistPeer::MEMBER_ID;
		}
	...
	public function setProfileId($v)
	{
		...
		if ($this->profile_id !== $v) {
			$this->profile_id = $v;
			$this->modifiedColumns[] = HotlistPeer::PROFILE_ID;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ($this->created_at !== $ts) {
			$this->created_at = $ts;
			$this->modifiedColumns[] = HotlistPeer::CREATED_AT;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			throw new PropelException("Error populating Hotlist object", $e);
		}
	}
    ...
    foreach (sfMixer::getCallables('BaseHotlist:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseHotlist:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseHotlist:save:pre') as $callable)
    {
    ...
    if ($this->isNew() && !$this->isColumnModified(HotlistPeer::CREATED_AT))
    {
      $this->setCreatedAt(time());
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseHotlist:save:post') as $callable)
    {
		...
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			...
			if ($this->aMemberRelatedByProfileId !== null) {
				...
				$this->setMemberRelatedByProfileId($this->aMemberRelatedByProfileId);
						...
						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = HotlistPeer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setId($pk);  
					$this->setNew(false);
				} else {
					$affectedRows += HotlistPeer::doUpdate($this, $con);
				}
				$this->resetModified(); 			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = HotlistPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = HotlistPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = HotlistPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(HotlistPeer::ID)) $criteria->add(HotlistPeer::ID, $this->id);
		if ($this->isColumnModified(HotlistPeer::MEMBER_ID)) $criteria->add(HotlistPeer::MEMBER_ID, $this->member_id);
		if ($this->isColumnModified(HotlistPeer::PROFILE_ID)) $criteria->add(HotlistPeer::PROFILE_ID, $this->profile_id);
		if ($this->isColumnModified(HotlistPeer::CREATED_AT)) $criteria->add(HotlistPeer::CREATED_AT, $this->created_at);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(HotlistPeer::ID, $this->id);
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseHotlist:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseHotlist::%s', $method));
    }
