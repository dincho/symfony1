...
<?php echo form_tag('dashboard/searchCriteria', array('id' => 'self_desc_form', 'name' => 'self_desc_form')) ?>
    <?php $i=1; ?>
    <span class="title_first"><?php echo $i; ?>. You're looking for someone aged: <span>(select one or more, as needed)</span></span>
    ...
    <?php foreach ($questions as $question): ?>
      ...
      <?php if( $question->getType() == 'radio' && isset($answers[$question->getId()]) ): ?>
        ...
        <span class="title"><?php echo ++$i . '|' . $question->getId(); ?>. <?php echo $question->getSearchTitle() ?> <span>(select one or more, as needed)</span></span>
      ...
      <?php elseif( $question->getType() == 'select' && isset($answers[$question->getId()]) ): ?>
        ...
        <span class="title"><?php echo ++$i; ?>. <?php echo $question->getSearchTitle() ?> <span>(select one)</span></span>
      ...
      <?php elseif( $question->getType() == 'native_lang' || $question->getType() == 'other_langs'): ?>
        ...
        <span class="title"><?php echo ++$i; ?>. <?php echo $question->getSearchTitle() ?></span>
