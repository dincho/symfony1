...
<?php echo form_tag($action, array('id' => 'messages_form_draft', 'name' => 'messages_form_draft', 'style' => 'display: none;')) ?>    
    ...
    <?php foreach ($draft_messages as $message): ?>
        ...
        <?php if (array_key_exists($message->getThreadId(), $msgCountByThread)):?>
            <?php $message->getThread()->setCntMessages($msgCountByThread[$message->getThreadId()]) ?>
            <?php endif;?>
        <?php else: ?>
